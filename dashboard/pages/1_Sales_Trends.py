...
class PredictorPlotter:
    ...
    def plot_prediction_history(self):
        ...
        c = hv.Curve((self.dfh.index, self.dfh.acv), label=f'Pacing to {final_val:0.1f}{self.units.upper()}').options(
            color='gray')
        return c
...
def plot_conversion(dfc, ylabel):
    ...
    for col, val in latest_vals.items():
        ...
        c = hv.Curve((dfc.index, dfc[col]), 'Date', ylabel, label=f'{col.title()} {actual}%').options(
            color=color, show_grid=True)
        c_list.append(c)
...
st.markdown('---')
st.markdown('## 12/31/22 Sales Forecast')
...
st.markdown('---')
st.markdown('## 30-day Rolling SAL Creation Rate')
...
df_sal_pretty = df_sal_pretty[[c for c in df_sal_pretty.columns if '_fit' not in c]]
...
st.markdown('---')
st.markdown('## Pipeline Conversion Rates')

st.markdown('### SAL to SQL')
